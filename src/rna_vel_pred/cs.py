...
class Dataset(CfgWithTable):
    ...
    time_step_count_sparsify: int = field(default=, metadata=dict(sa=ColumnRequired(sa.Integer)))
    neighbor_count: int = field(default=, metadata=dict(sa=ColumnRequired(sa.Integer)))

    frac_train: float = field(default=, metadata=dict(sa=ColumnRequired(sa.Double)))
    frac_val: float = field(default=, metadata=dict(sa=ColumnRequired(sa.Double)))
    frac_test: float = field(default=, metadata=dict(sa=ColumnRequired(sa.Double)))
    batch_size: int = field(default=, metadata=dict(sa=ColumnRequired(sa.Integer)))

    limit_batch_count_train: bool = field(default=, metadata=dict(sa=ColumnRequired(sa.Integer)))
    batch_count_train: bool = field(default=, metadata=dict(sa=ColumnRequired(sa.Integer)))

    reverse_velocities: bool = field(default=False, metadata=dict(sa=ColumnRequired(sa.Boolean)))


class DatasetMotif(Dataset):
    __tablename__ = __qualname__
    __mapper_args__ = dict(
        polymorphic_on='sa_inheritance',
        polymorphic_identity=__tablename__,
    ...
    id: int = field(init=False, metadata=dict(
        sa=sa.Column(sa.ForeignKey(f'{DatasetMotif.__name__}.id'), primary_key=True),
        omegaconf_ignore=True,
    ))

    data_count: int = field(default=4000, metadata=dict(sa=ColumnRequired(sa.Integer)))
    epsilon: float = field(default=..., metadata=dict(sa=ColumnRequired(sa.Double)))


class DatasetSimple(DatasetMotif):
    __tablename__ = __qualname__
    __mapper_args__ = dict(
        polymorphic_on='sa_inheritance',
        polymorphic_identity=__tablename__,
...
class DatasetOscillation(DatasetMotif):
    __tablename__ = __qualname__
    __mapper_args__ = dict(
        polymorphic_on='sa_inheritance',
        polymorphic_identity=__tablename__,
...
class DatasetBifurcation(DatasetMotif):
    __tablename__ = __qualname__
    __mapper_args__ = dict(
        polymorphic_on='sa_inheritance',
        polymorphic_identity=__tablename__,
...
class DatasetForUMap(Dataset):
    ...
    dataset: UMapDataset = field(default=omegaconf.MISSING, metadata=dict(ColumnRequired(sa.Enum(UMapDataset))))

    data_dir: str = field(default=omegaconf.MISSING, metadata=dict(sa=ColumnRequired(sa.String(40))))
    filename_h5ad: str = field(default=omegaconf.MISSING, metadata=dict(sa=ColumnRequired(sa.String(40))))
    filename_processed: str = field(default=omegaconf.MISSING, metadata=dict(sa=ColumnRequired(sa.String(40))))

    umap_dimension: int = field(default=2, metadata=dict(sa=ColumnRequired(sa.Integer())))
...
class Config(CfgWithTable):
    ...
    datasets: typing.List[typing.Any] = field(default_factory=list, metadata=dict(
        sa=orm.relationship(Dataset.__name__, secondary=lambda: table_m2m_Config__Dataset)
    ))
...
table_m2m_Config__Dataset = sa.Table(
    f'{Config.__name__}__{Dataset.__name__}',
    mapper_registry.metadata,
    sa.Column(Config.__name__, sa.ForeignKey(f'{Config.__name__}.id'), primary_key=True),
    sa.Column(Dataset.__name__, sa.ForeignKey(f'{Dataset.__name__}.id'), primary_key=True),
)
